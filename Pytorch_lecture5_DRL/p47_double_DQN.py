...
# Google DeepMind, Hasselt, AAAI 2016
...
class DoubleDQN(object):
    ...
    def train(self):
        ...
        if self.learn_step_counter % TARGET_REPLACE_ITER == 0: # hard update
            self.target_net.load_state_dict(self.eval_net.state_dict())
        # self.soft_update(self.target_net,self.eval_net) # soft update
...
for i in range(EPOCHS):
    ...
    while True:
        ...
        if ddqn.memory_counter > BATCH_SIZE:
            losses.append(ddqn.train()) # experience replay
