...
class GRU(torch.nn.Module):
    def __init__(self):
        ...
        self.rnn = torch.nn.GRU(input_size=1, hidden_size=32, num_layers=1, batch_first=True)
...
torch.save(model.state_dict(),'./checkpoint/gru.pth')
...
model.load_state_dict(torch.load('./checkpoint/gru.pth'))
ybar,_ = model(x, None)
